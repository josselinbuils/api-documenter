...
export class YamlDocumenter {
  ...
  private _generateYamlItem(apiItem: ApiDocumentedItem): IYamlItem | undefined {
    ...
    if (apiItem.tsdocComment) {
      ...
      if (tsdocComment.summarySection) {
        const summary: string = this._renderMarkdown(
      ...
      if (tsdocComment.remarksBlock) {
        const remarks: string = this._renderMarkdown(
    ...
    switch (apiItem.kind) {
      ...
      case ApiItemKind.MethodSignature:
        ...
        this._populateYamlFunctionLike(yamlItem, apiItem as
          | ApiMethod
          | ApiMethodSignature);
        break;
    ...
    if (
      ...
      const associatedPackage:
        | ApiPackage
        | undefined = apiItem.getAssociatedPackage();
      if (!associatedPackage) {
        throw new Error(
          'Unable to determine associated package for ' + apiItem.displayName
        );
      }
  ...
  private _populateYamlClassOrInterface(
    ...
    if (apiItem instanceof ApiClass) {
      if (apiItem.extendsType) {
        yamlItem.extends = [
          this._linkToUidIfPossible(apiItem.extendsType.excerpt.text)
        ];
      }
      if (apiItem.implementsTypes.length > 0) {
        yamlItem.implements = [];
        for (const implementsType of apiItem.implementsTypes) {
          yamlItem.implements.push(
            this._linkToUidIfPossible(implementsType.excerpt.text)
          );
        }
      }
      ...
      if (apiItem.extendsTypes.length > 0) {
        ...
        for (const extendsType of apiItem.extendsTypes) {
          yamlItem.extends.push(
            this._linkToUidIfPossible(extendsType.excerpt.text)
          );
        }
      }
    }
  ...
  private _populateYamlFunctionLike(
    ...
    if (ApiReturnTypeMixin.isBaseClassOf(apiItem)) {
      const returnType: string = this._linkToUidIfPossible(
        apiItem.returnTypeExcerpt.text
      );
    ...
    for (const apiParameter of apiItem.parameters) {
      ...
      if (apiParameter.tsdocParamBlock) {
        parameterDescription = this._renderMarkdown(
      ...
      parameters.push({
        ...
        type: [
          this._linkToUidIfPossible(apiParameter.parameterTypeExcerpt.text)
        ]
      } as IYamlParameter);
    }
   ...
   */
  private _initApiItemsByTypeNameRecursive(
    apiItem: ApiItem,
    ...
    switch (apiItem.kind) {
      ...
      case ApiItemKind.Interface:
        ...
        const nameWithDot: string | undefined = this._getTypeNameWithDot(
          apiItem
        );
        if (nameWithDot) {
  ...
  private _linkToUidIfPossible(typeName: string): string {
    ...
    const apiItem: ApiItem | undefined = this._apiItemsByTypeName.get(
      typeName.trim()
    );
    if (apiItem) {
      // Substitute the UID
    ...
    for (const current of apiItem.getHierarchy()) {
      switch (current.kind) {
        ...
        default:
          if (
            current.parent &&
            current.parent.kind === ApiItemKind.EntryPoint
          ) {
            result += '/';
          } else {
            result += '.';
